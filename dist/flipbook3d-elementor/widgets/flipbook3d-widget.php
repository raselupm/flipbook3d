...
class FlipBook3D_Widget extends Widget_Base {
	...
	public function get_title()      { return esc_html__( 'FlipBook3D', 'flipbook3d-elementor' ); }
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_source', [
			'label' => esc_html__( 'Content Source', 'flipbook3d-elementor' ),
			'tab'   => Controls_Manager::TAB_CONTENT,
		] );

		$this->add_control( 'source_type', [
			'label'   => esc_html__( 'Source Type', 'flipbook3d-elementor' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'pdf_url',
			'options' => [
				'pdf_url'   => esc_html__( 'PDF — External URL', 'flipbook3d-elementor' ),
				'pdf_media' => esc_html__( 'PDF — Media Library', 'flipbook3d-elementor' ),
				'images'    => esc_html__( 'Image Gallery', 'flipbook3d-elementor' ),
			],
		] );

		// -- PDF: External URL --
		$this->add_control( 'pdf_url', [
			'label'         => esc_html__( 'PDF URL', 'flipbook3d-elementor' ),
			'type'          => Controls_Manager::URL,
			'placeholder'   => 'https://example.com/document.pdf',
			'show_external' => false,
			'default'       => [ 'url' => '' ],
			'condition'     => [ 'source_type' => 'pdf_url' ],
			'description'   => esc_html__( 'Paste the full URL of your PDF. The server must allow cross-origin access (CORS).', 'flipbook3d-elementor' ),
		] );

		// -- PDF: Media Library --
		$this->add_control( 'pdf_media', [
			'label'       => esc_html__( 'PDF File', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::MEDIA,
			'media_types' => [ 'application/pdf' ],
			'condition'   => [ 'source_type' => 'pdf_media' ],
			'description' => esc_html__( 'Upload or select a PDF from your Media Library.', 'flipbook3d-elementor' ),
		] );

		// -- Images --
		$this->add_control( 'image_gallery', [
			'label'       => esc_html__( 'Image Gallery', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::GALLERY,
			'condition'   => [ 'source_type' => 'images' ],
			'description' => esc_html__( 'Each image becomes one page. Use images with consistent dimensions for the best result.', 'flipbook3d-elementor' ),
		] );
		...
		$this->start_controls_section( 'section_dimensions', [
			'label' => esc_html__( 'Book Dimensions', 'flipbook3d-elementor' ),
			'tab'   => Controls_Manager::TAB_CONTENT,
		] );

		$this->add_control( 'book_width', [
			'label'       => esc_html__( 'Book Width (px)', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::SLIDER,
			'size_units'  => [ 'px' ],
			'range'       => [ 'px' => [ 'min' => 300, 'max' => 2000, 'step' => 10 ] ],
			'default'     => [ 'size' => 900, 'unit' => 'px' ],
			'description' => esc_html__( 'Total width of the open book (both pages combined). The book will be automatically capped to its container width.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'aspect_ratio_preset', [
			'label'   => esc_html__( 'Page Aspect Ratio', 'flipbook3d-elementor' ),
			'type'    => Controls_Manager::SELECT,
			'default' => '1.414',
			'options' => [
				'1.414'  => esc_html__( 'A4 Portrait (1.414)', 'flipbook3d-elementor' ),
				'1.294'  => esc_html__( 'US Letter Portrait (1.294)', 'flipbook3d-elementor' ),
				'0.707'  => esc_html__( 'A4 Landscape (0.707)', 'flipbook3d-elementor' ),
				'0.773'  => esc_html__( 'US Letter Landscape (0.773)', 'flipbook3d-elementor' ),
				'1.0'    => esc_html__( 'Square (1.0)', 'flipbook3d-elementor' ),
				'1.5'    => esc_html__( 'Magazine (1.5)', 'flipbook3d-elementor' ),
				'custom' => esc_html__( 'Custom…', 'flipbook3d-elementor' ),
			],
			'description' => esc_html__( 'Height ÷ width ratio per page. Match this to your PDF or images.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'aspect_ratio_custom', [
			'label'     => esc_html__( 'Custom Ratio (height ÷ width)', 'flipbook3d-elementor' ),
		...
		$this->start_controls_section( 'section_animation', [
			'label' => esc_html__( 'Animation & Rendering', 'flipbook3d-elementor' ),
			'tab'   => Controls_Manager::TAB_CONTENT,
		] );

		$this->add_control( 'flip_duration', [
			'label'       => esc_html__( 'Flip Duration (ms)', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::SLIDER,
			'size_units'  => [ 'px' ],
			'range'       => [ 'px' => [ 'min' => 100, 'max' => 2000, 'step' => 50 ] ],
			'default'     => [ 'size' => 700, 'unit' => 'px' ],
			'description' => esc_html__( 'Duration of the 3D page-turn animation in milliseconds. Lower = snappier; higher = more dramatic.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'render_scale', [
			'label'       => esc_html__( 'Render Scale', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::SLIDER,
			'size_units'  => [ 'px' ],
			'range'       => [ 'px' => [ 'min' => 0.5, 'max' => 3.0, 'step' => 0.1 ] ],
			'default'     => [ 'size' => 1.5, 'unit' => 'px' ],
			'description' => esc_html__( 'Internal canvas render scale. Higher = sharper pages but more GPU/RAM usage. 1.5–2 is the sweet spot for most screens.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'page_background', [
			'label'       => esc_html__( 'Page Background Colour', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::COLOR,
			'default'     => '#ffffff',
			'description' => esc_html__( 'Colour shown behind page content (fill for transparent images or PDFs with a white canvas).', 'flipbook3d-elementor' ),
		] );
		...
		$this->start_controls_section( 'section_behaviour', [
			'label' => esc_html__( 'Controls & Behaviour', 'flipbook3d-elementor' ),
			'tab'   => Controls_Manager::TAB_CONTENT,
		] );

		$this->add_control( 'start_page', [
			'label'       => esc_html__( 'Start Page', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::NUMBER,
			'min'         => 1,
			'max'         => 9999,
			'step'        => 1,
			'default'     => 1,
			'description' => esc_html__( 'Which page to open when the book first loads (1 = first page).', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'mute_sound', [
			'label'        => esc_html__( 'Page-Flip Sound', 'flipbook3d-elementor' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'Muted', 'flipbook3d-elementor' ),
			'label_off'    => esc_html__( 'Enabled', 'flipbook3d-elementor' ),
			'return_value' => 'yes',
			'default'      => '',
			'description'  => esc_html__( 'The page-flip sound is synthesised in the browser via Web Audio API. Mute it here if unwanted.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'show_controls', [
			'label'        => esc_html__( 'Navigation Bar', 'flipbook3d-elementor' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'Visible', 'flipbook3d-elementor' ),
			'label_off'    => esc_html__( 'Hidden', 'flipbook3d-elementor' ),
			'return_value' => 'yes',
			'default'      => 'yes',
			'description'  => esc_html__( 'Show or hide the bottom bar with First / Prev / Next / Last buttons.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'show_page_numbers', [
			'label'        => esc_html__( 'Page Number Overlay', 'flipbook3d-elementor' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'Visible', 'flipbook3d-elementor' ),
			'label_off'    => esc_html__( 'Hidden', 'flipbook3d-elementor' ),
			'return_value' => 'yes',
			'default'      => 'yes',
			'description'  => esc_html__( 'Small page-number label shown in the corner of each page.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'show_fullscreen', [
			'label'        => esc_html__( 'Fullscreen Button', 'flipbook3d-elementor' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'Visible', 'flipbook3d-elementor' ),
			'label_off'    => esc_html__( 'Hidden', 'flipbook3d-elementor' ),
			'return_value' => 'yes',
			...
			'description'  => esc_html__( 'Show the fullscreen toggle inside the navigation bar.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'click_to_flip', [
			'label'        => esc_html__( 'Click Page to Flip', 'flipbook3d-elementor' ),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__( 'Enabled', 'flipbook3d-elementor' ),
			'label_off'    => esc_html__( 'Disabled', 'flipbook3d-elementor' ),
			'return_value' => 'yes',
			'default'      => 'yes',
			'description'  => esc_html__( 'Allow clicking on the left/right page edges to flip pages.', 'flipbook3d-elementor' ),
		] );
		...
		$this->start_controls_section( 'section_colors', [
			'label' => esc_html__( 'Theme Colours', 'flipbook3d-elementor' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );

		$this->add_control( 'color_accent', [
			'label'       => esc_html__( 'Accent Colour', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::COLOR,
			'default'     => '#e8c87d',
			'selectors'   => [ '{{WRAPPER}} .flipbook3d-wrapper' => '--fb-accent: {{VALUE}};' ],
			'description' => esc_html__( 'Used for button borders, hover highlights, and arrow indicators.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'color_spine', [
			'label'       => esc_html__( 'Spine Colour', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::COLOR,
			'default'     => '#c8a96e',
			'selectors'   => [ '{{WRAPPER}} .flipbook3d-wrapper' => '--fb-spine-color: {{VALUE}};' ],
			'description' => esc_html__( 'The vertical book spine strip at the centre of the spread.', 'flipbook3d-elementor' ),
		] );

		$this->add_control( 'color_page_bg', [
			'label'       => esc_html__( 'Page Background (CSS var)', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::COLOR,
			'default'     => '#ffffff',
			'selectors'   => [ '{{WRAPPER}} .flipbook3d-wrapper' => '--fb-page-bg: {{VALUE}};' ],
			'description' => esc_html__( 'CSS variable counterpart of the Page Background setting. Both should match.', 'flipbook3d-elementor' ),
		] );
		...
		$this->add_control( 'color_ui_bg', [
			'label'       => esc_html__( 'Bar Background', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::COLOR,
		...
		$this->add_control( 'color_ui_text', [
			'label'       => esc_html__( 'Bar Text Colour', 'flipbook3d-elementor' ),
			'type'        => Controls_Manager::COLOR,
			'default'     => '#f0e8d8',
		...
		$this->start_controls_section( 'section_typography', [
			'label' => esc_html__( 'Typography', 'flipbook3d-elementor' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );

		$this->add_control( 'btn_font_size', [
			'label'      => esc_html__( 'Button Font Size', 'flipbook3d-elementor' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [ 'px' ],
		...
		$this->add_control( 'page_num_font_size', [
			'label'      => esc_html__( 'Page Number Font Size', 'flipbook3d-elementor' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [ 'px' ],
		...
		$this->start_controls_section( 'section_controls_style', [
			'label'     => esc_html__( 'Navigation Bar Style', 'flipbook3d-elementor' ),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [ 'show_controls' => 'yes' ],
		] );

		$this->add_control( 'controls_border_radius', [
			'label'      => esc_html__( 'Bar Border Radius', 'flipbook3d-elementor' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [ 'px' ],
		...
		$this->add_control( 'controls_padding', [
			'label'      => esc_html__( 'Bar Padding', 'flipbook3d-elementor' ),
		...
		$this->start_controls_section( 'section_wrapper', [
			'label' => esc_html__( 'Widget Wrapper', 'flipbook3d-elementor' ),
			'tab'   => Controls_Manager::TAB_STYLE,
		] );

		$this->add_responsive_control( 'wrapper_align', [
			'label'   => esc_html__( 'Alignment', 'flipbook3d-elementor' ),
			'type'    => Controls_Manager::CHOOSE,
			'options' => [
				'flex-start' => [ 'title' => esc_html__( 'Left', 'flipbook3d-elementor' ),   'icon' => 'eicon-text-align-left' ],
				'center'     => [ 'title' => esc_html__( 'Center', 'flipbook3d-elementor' ), 'icon' => 'eicon-text-align-center' ],
				'flex-end'   => [ 'title' => esc_html__( 'Right', 'flipbook3d-elementor' ),  'icon' => 'eicon-text-align-right' ],
			],
			'default'   => 'center',
		...
		$this->add_responsive_control( 'wrapper_padding', [
			'label'      => esc_html__( 'Padding', 'flipbook3d-elementor' ),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => [ 'px', 'em', '%' ],
		...
		$this->add_control( 'wrapper_bg', [
			'label'     => esc_html__( 'Background Colour', 'flipbook3d-elementor' ),
			'type'      => Controls_Manager::COLOR,
		...
		$this->add_group_control( Group_Control_Box_Shadow::get_type(), [
			'name'     => 'book_shadow',
			'label'    => esc_html__( 'Book Box Shadow', 'flipbook3d-elementor' ),
			...
			>
				<?php if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) :
					...
					if ( $pdf_url ) {
						...
						$meta = sprintf( esc_html__( '%d page images', 'flipbook3d-elementor' ), count( $image_urls ) );
					} else {
						$meta = esc_html__( 'Select a PDF or images in the panel →', 'flipbook3d-elementor' );
					}
					$dims = sprintf(
						/* translators: 1: width in px, 2: aspect ratio */
						esc_html__( 'Width: %1$dpx · Aspect: %2$s', 'flipbook3d-elementor' ),
				...
				?>
					<div class="fb3d-editor-placeholder">
						...
						<span class="fb3d-title"><?php esc_html_e( 'FlipBook3D', 'flipbook3d-elementor' ); ?></span>
